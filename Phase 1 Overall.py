...
description_dict = {'Solar': ["solar", "solar panel", "photovoltaic", "array"],                        # Dict of keyphrases(the values) that match the desired label(the keys)
...
def ExtractMetaData(csvfilename):																# ExtractMetaData function
    ...
    with open("sample_data/" + filename, 'rb') as csvfile:
        ...
        for chunk in MD:																		# Iterates through objects in top line of array
        	lower_chunk = chunk.lower()
        	clean_chunk = lower_chunk.replace(" [kw]", "")
        	description_list.append(clean_chunk)												# Adds items to description_list
     
        	for key in inv_dict:			######## Sometimes Maps Wrong Description to Wrong Label ########Iterates through keys in the inv_dict dictionary
        		if key in clean_chunk:															# Checks for keyphrase matches
    	...
    	for z in range(len(label_list)):
        	...
        	MetaData_list.append(m)																# Creates list of desired MetaData objects for each circuit existing in the eGauge
...
def writeDeviceInfo(egauge_name, metadata):
	...
	with open("sample_data/" + egauge_name, 'rb') as csvFile:									# Converts the un-readable csv file into a more read able form
		...
		if not os.path.exists(clean_egauge):													
			os.makedirs(clean_egauge)															# Creates new folder for eGauge
			os.chdir(orig_dir)
		else:
			os.chdir(orig_dir)

		file_list = []
		for column in xrange(len(skip_line[1:])):												# Iterates through the columns in the number of columns that exist
